...
[  --with-php-psutil[=DIR]       Set the path to cpslib])
...
if test -r "$PHP_PHP_PSUTIL/libpslib.so" && test -r "$PHP_PHP_PSUTIL/pslib.h"; then
    ...
    AC_MSG_ERROR([Must set the path to cpslib. Use --with-php-psutil=<DIR> to specify the location of the header and library files])
fi
